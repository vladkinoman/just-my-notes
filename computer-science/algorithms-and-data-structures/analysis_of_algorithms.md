...
## Шаги для разработки юзабильного алгоритма

1. Смоделировать задачу (problem).
2. Найти алгоритм чтобы решить её.
3. Достаточно быстрый? Влезает в память?
4. Если нет, то понять почему.
5. Найти способ адресовать (решить) эту проблему.
6. Повторям пока не удовлетворится.
